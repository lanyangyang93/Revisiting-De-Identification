...
class TestMetric(object):
    ...
    def test_conll2000(self):
        ...
        gold_data = ConllIO(text_col_id=0, tag_col_id=2, scheme='BIO2').read("output.txt")
        pred_data = ConllIO(text_col_id=0, tag_col_id=3, scheme='BIO2').read("output.txt")
        ...
        expected_ave_scores = {'macro': {'precision': 0.54880502,
                               ...
                               'micro': {'precision': 0.68831169, 
                                         'recall': 0.80827887, 
                                         'f1': 0.74348697}}
        print(precision_recall_f1_report(chunks_gold_data, chunks_pred_data))
        scores, ave_scores = precision_recall_f1_report(chunks_gold_data, chunks_pred_data)
        self._assert_scores_equal(ave_scores, expected_ave_scores)
