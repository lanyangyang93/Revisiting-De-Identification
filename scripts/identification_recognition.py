...
from eznlp.dataset import Dataset, BatchSchedulerSampler, Word2vcDataset, LableDataset
from eznlp.config import ConfigDict
from eznlp.model import OneHotConfig, MultiHotConfig, EncoderConfig, CharConfig, SoftLexiconConfig, SourceHotConfig
...
def parse_arguments(parser: argparse.ArgumentParser):
    ...
    group_data.add_argument('--use_lable', default=False, action='store_true',
                            help="whether use lable ")
    group_data.add_argument('--use_word2vec', default=False, action='store_true',
                            help="whether use word2vec ")
    group_data.add_argument('--weight_decay', default=1e-2, type=float,
                            help="whether weight_decay ")
    group_data.add_argument('--text_smoothing',  default=False, action='store_true',
                            help="whether text smoothing ")
    group_data.add_argument('--save_every_epoch', default=False, action='store_true',
                            help="whether save every epoch ")
...
def collect_IE_assembly_config(args: argparse.Namespace):
    ...
    ohots_config = {}
    if args.emb_dim > 0 :
        vectors = load_vectors(args.language, args.emb_dim, unigram=True, use_word2vec=args.use_word2vec)
    ...
    if args.language.lower() == 'chinese' and args.use_source:
        source_config = ConfigDict({'source': SourceHotConfig(field='source')})
    else:
        source_config = None
    ...
    if args.use_interm2:
        interm2_config = EncoderConfig(arch=args.enc_arch, hid_dim=args.hid_dim, num_layers=args.num_layers, in_drop_rates=drop_rates, kernel_size=args.kernel_size)
    else:
        interm2_config = None
...
if __name__ == '__main__':
    ...
    if args.random_replace_ratio < 1.0:
        logger.info(f"The train dataset will replacement occurs and {(1-args.random_replace_ratio)*100}%")

    if not args.train_with_dev:
        if args.use_lable:
            train_set = LableDataset(train_data, config, training=True, ratio=args.random_replace_ratio)
            train_set.build_vocabs_and_dims(dev_data, test_data)
            dev_set = LableDataset(dev_data, train_set.config, training=False)
            test_set = LableDataset(test_data, train_set.config, training=False)
            ...
            train_set = Dataset(train_data, config, training=True, ratio=args.random_replace_ratio)
            train_set.build_vocabs_and_dims(dev_data, test_data)
            dev_set   = Dataset(dev_data,  train_set.config, training=False)
            test_set  = Dataset(test_data, train_set.config, training=False)

            train_loader = torch.utils.data.DataLoader(train_set, batch_size=args.batch_size, shuffle=False,  collate_fn=train_set.collate)
            dev_loader   = torch.utils.data.DataLoader(dev_set,   batch_size=args.batch_size, shuffle=False, collate_fn=dev_set.collate)
    else:
        train_set = Dataset(train_data + dev_data, config, training=True, ratio=args.random_replace_ratio)
    ...
    if args.overfit:
        logger.info('train will overfit')
        train_set = Dataset(train_data, config, training=True, ratio=args.random_replace_ratio)
        train_set.build_vocabs_and_dims(test_data)
        dev_set = Dataset([], train_set.config, training=False)
        test_set = Dataset(test_data, train_set.config, training=False)
    ...
    def save_callback(model,i):
        torch.save(model, f"{save_path}/{config.name}_epoch_{i}.pth")
    trainer.train_steps(train_loader=train_loader, dev_loader=dev_loader, num_epochs=args.num_epochs,
                        save_callback=save_callback, save_every_epoch=args.save_every_epoch, save_by_loss=False)
    
    # Save the final version
    # torch.save(model, f"{save_path}/{config.name}.fv.pth")
    
    logger.info(header_format("Evaluating", sep='-'))
    model = torch.load(f"{save_path}/{config.name}_epoch_best.pth", map_location=device)
    trainer = Trainer(model, device=device)
    
    logger.info("Evaluating on dev-set")
    evaluate_entity_recognition(trainer, dev_set, batch_size=args.batch_size)
    if args.post_processing:
        logger.info("Evaluating on test-set with post-processing")
    logger.info("Evaluating on test-set")
    evaluate_entity_recognition(trainer, test_set, batch_size=args.batch_size, save_preds=args.save_preds, post_processing=args.post_processing)
    if args.save_preds:
