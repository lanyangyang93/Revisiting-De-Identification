...
def add_base_arguments(parser: argparse.ArgumentParser):
    ...
    group_train.add_argument('--optimizer', type=str, default='AdamW', 
                             help="optimizer", choices=['AdamW', 'SGD', 'Adadelta', 'Adamax', 'Adam'])
    ...
    group_model.add_argument('--kernel_size', type=int, default=15,
                             help="kernel_size encoder Conv architecture")
...
dataset2language = {'conll2003': 'English', 
                    ...
                    'mscoco': 'English',
                    'HwaMei2':'Chinese',
                    'HwaMei_Privacy':'Chinese',
                    'ShaoYang_Privacy':'Chinese',
                    'CCKS_Privacy':'Chinese'
                    }
...
def load_data(args: argparse.Namespace):
    ...
    elif args.dataset == 'HwaMei_Privacy':
        io = JsonIO(text_key='tokens', chunk_key='entities', chunk_type_key='type', chunk_start_key='start',
        chunk_end_key='end', is_whole_piece = False, chunk_text_key=None, encoding = 'utf-8',retain_meta=True)
        train_data = io.read("data/HwaMei_Privacy/privacy-train-deid.json")
        dev_data = io.read("data/HwaMei_Privacy/privacy-dev-deid.json")
        test_data = io.read("data/HwaMei_Privacy/privacy-test-deid.json")



    elif args.dataset == 'ShaoYang_Privacy':
        io = JsonIO(text_key='tokens', chunk_key='entities', chunk_type_key='type', chunk_start_key='start',
        chunk_end_key='end',  is_whole_piece = False, chunk_text_key=None, encoding = 'utf-8',retain_meta=True)
        train_data = io.read("data/HwaMei_Privacy/privacy-train-deid.json")
        dev_data = io.read("data/HwaMei_Privacy/privacy-dev-deid.json")
        test_data = io.read("data/ShaoYang_Privacy/privacy-shaoyang-v1-deid.json")



    elif args.dataset == 'CCKS_Privacy':
        io = JsonIO(text_key='tokens', chunk_key='entities', chunk_type_key='type', chunk_start_key='start',
                    chunk_end_key='end', is_whole_piece=False, chunk_text_key=None, encoding='utf-8', retain_meta=True)
        train_data = io.read("data/HwaMei_Privacy/privacy-train-deid.json")
        dev_data = io.read("data/HwaMei_Privacy/privacy-dev-deid.json")
        test_data = io.read("data/CCKS2017_Privacy/privacy-ccks2017-v1.json")
...
def load_pretrained(pretrained_str, args: argparse.Namespace, cased=False):
    if pretrained_str.lower() == 'elmo':
        ...
        if pretrained_str.lower().startswith('bert'):
            ...
            elif 'hm-wmm-15m' in pretrained_str.lower():
                PATH = "assets/transformers/bert-base-hm-wmm-20e-384b-15m"
            elif 'mc' in pretrained_str.lower():
                PATH = "assets/transformers/mc_bert_base"
            else:
                PATH = "assets/transformers/bert-base-chinese"
            if args.text_smoothing:
                return (transformers.BertModel.from_pretrained(PATH, hidden_dropout_prob=args.bert_drop_rate,
                        ...
                        transformers.BertForMaskedLM.from_pretrained(PATH))
            return (transformers.BertModel.from_pretrained(PATH, hidden_dropout_prob=args.bert_drop_rate, attention_probs_dropout_prob=args.bert_drop_rate), 
                    transformers.BertTokenizer.from_pretrained(PATH, model_max_length=512, do_lower_case=True))
...
def load_vectors(language: str, emb_dim: int, unigram: bool=False, bigram: bool=True, use_word2vec:bool=False):
    if language.lower() == 'english':
        if emb_dim in (50, 100, 200):
            return GloVe(f"assets/vectors/glove.6B.{emb_dim}d.txt", encoding='utf-8')
        elif emb_dim == 300:
        ...
        if unigram and emb_dim == 50:
            ...
            elif emb_dim == 100 and use_word2vec:
                return Vectors.load("assets/vectors/word2vec_15.txt", encoding='utf-8', skiprows=0)
    return None
...
def build_trainer(model, device, num_train_batches: int, args: argparse.Namespace):
    param_groups = [{'params': model.pretrained_parameters(), 'lr': args.finetune_lr, 'weight_decay': args.weight_decay}]
    param_groups.append({'params': collect_params(model, param_groups), 'lr': args.lr, 'weight_decay': args.weight_decay})
    # param_groups[1]['params'][11].requires_grad = False
    # param_groups[1]['params'][12].requires_grad = False
    # param_groups[1]['params'][13].requires_grad = False
    assert check_param_groups(model, param_groups)
    optimizer = getattr(torch.optim, args.optimizer)(param_groups)  #给定参数 否则默认返回指定参数
    ...
    return Trainer(model, optimizer=optimizer, scheduler=scheduler, schedule_by_step=schedule_by_step, num_grad_acc_steps=args.num_grad_acc_steps,
                       device=device, grad_clip=args.grad_clip, use_amp=args.use_amp, coefficient=args.coefficient, text_smoothing=args.text_smoothing)
    # return Trainer(model, optimizer=optimizer, scheduler=scheduler, schedule_by_step=schedule_by_step, num_grad_acc_steps=args.num_grad_acc_steps,
