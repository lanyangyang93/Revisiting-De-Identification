...
class SequenceTaggingDecoderMixin(DecoderMixinBase):
    ...
    def retrieve(self, batch: Batch):
        return [tags_obj.chunks for tags_obj in batch.tags_objs if tags_obj.chunks is not None]
...
class Tags(TargetWrapper):
    ...
    def __init__(self, data_entry: dict, config: SequenceTaggingDecoderMixin, training: bool=True):
        ...
        self.lable = data_entry.get('orgin_tokenized_text', None)
        self.chunks = data_entry.get('chunks', None)
        self.version = data_entry.get('version', None)
        if self.chunks is not None:
            if self.lable is not None:
                self.tags = config.translator.chunks2tags(data_entry['chunks'], len(data_entry['orgin_tokenized_text']))
            else:
...
class SequenceTaggingDecoder(DecoderBase, SequenceTaggingDecoderMixin):
    ...
    def forward(self, batch: Batch, full_hidden: torch.Tensor):
        ...
        # if isinstance(batch, orgin_tokenized_text):
        #     logits = self.hid2logit(self.dropout(full_hidden))[:, 1:]
        ...
        if isinstance(self.criterion, CRF):
            ...
            #无标签置0
            losses = [self.criterion(lg[:slen], tags_obj.tag_ids) if tags_obj.chunks is not None else self.criterion(lg[:slen], torch.zeros(slen,dtype=torch.long).to(device))
                      for lg, tags_obj, slen in zip(logits, batch.tags_objs, batch.seq_lens.cpu().tolist()) ]
    ...
    def decode_tags(self, batch: Batch, full_hidden: torch.Tensor):
        ...
        # with open('/home/lyy/eznlp/scripts/embedding/SY_embedd_hm_bert.txt','a+',encoding='utf-8') as f:
        #     a = torch.mean(logits,dim=1)
        #     for b in a.tolist():
        #         f.write(' '.join('%s' %id for id in b))
        #         f.write('\n')
        ...
        # [[self.idx2tag[i] for i in tag_ids] if tags.chunks not is None else continue
        # for tag_ids, tags in zip(batch_tag_ids, batch.tags_objs)]
        return [[self.idx2tag[i] for i in tag_ids] if tags.chunks is not None else None
        for tag_ids, tags in zip(batch_tag_ids, batch.tags_objs)]
        
        
    def decode(self, batch: Batch, full_hidden: torch.Tensor):
        batch_tags = self.decode_tags(batch, full_hidden)
        return [self.translator.tags2chunks(tags) for tags in batch_tags if tags is not None]
