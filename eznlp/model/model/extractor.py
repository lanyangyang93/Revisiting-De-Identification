...
class ExtractorConfig(ModelConfigBase):
    ...
    _embedder_names = ['ohots', 'mhots', 'nested_ohots', 'source']
    ...
    def build_vocabs_and_dims(self, *partitions):
        ...
        if self.source is not None:
            for c in self.source.values():
                c.build_lable(*partitions)
    ...
    def exemplify(self, data_entry: dict, training: bool=True):
        ...
        if self.source is not None:
            title = data_entry['doc_id'].split('/')[1]
            title = re.findall(r'[a-zA-Z]+', title)[0]
            example['source'] = {f: c.exemplify(title) for f, c in self.source.items()}
    ...
    def batchify(self, batch_examples: List[dict]):
        ...
        if self.source is not None:
            batch['source'] = {f: c.batchify([ex['source'][f] for ex in batch_examples]) for f, c in self.source.items()}
...
class Extractor(ModelBase):
    ...
    def _get_full_embedded(self, batch: Batch):
        ...
        if hasattr(self, 'source'):
            source_ohots_embedded = [self.source[f](batch.source[f]) for f in self.source]
            max_seq_len = ohots_embedded[0].shape[1]
            source_ohots_embedded = [source_ohots_embedded[0].repeat(1, max_seq_len, 1)]
            embedded.extend(source_ohots_embedded)
        
        return torch.cat(embedded, dim=-1)

    def _get_word2vec_embedded(self):
        # embedded = []
        #
        # for sents in batch.skipgam:
        #     for f in sents:
        #         pos_embedded = self.ohots['text'](sents[0].to(device))
        #         center_embedded = self.ohots['text'](f[1].to(device))
        #         neg_embedded = self.ohots['text'](f[2].to(device))
        #         embedded.append((center_embedded,pos_embedded,neg_embedded))
        return self.ohots['text']
    ...
    def _get_full_hidden(self, batch: Batch):
        ...
        if hasattr(batch,'orgin_tokenized_text'):
            index = len(batch.tokenized_text[0]) - len(batch.orgin_tokenized_text[0])
            full_hidden = full_hidden[:,index:]
