...
class SourceHotConfig(Config):
    """Config of a multi-hot embedder.
    """

    def __init__(self, **kwargs):
        self.field = kwargs.pop('field','source')
        self.emb_dim = kwargs.pop('emb_dim', 100)
        self.max_len = 1
        self.min_freq= 1
        super().__init__(**kwargs)
    ...
    def build_lable(self, *partitions):
        counter = Counter()
        for data in partitions:
            for entry in data:
                title = entry['doc_id'].split('/')[1]
        ...
        self.lable = Vocab(counter, min_freq=self.min_freq, specials_first=True)

    def exemplify(self, tokens: TokenSequence):

        return torch.tensor(self.lable[tokens], dtype=torch.long)

    def batchify(self, batch_values: List[torch.FloatTensor]):
        return torch.tensor(batch_values).view(-1,1)

    def instantiate(self):
        return SourceHotEmbedder(self)



class SourceHotEmbedder(torch.nn.Module):
    def __init__(self, config: SourceHotConfig):
        super().__init__()
        self.embedding = torch.nn.Embedding(len(config.lable),config.emb_dim)
